...
const Perfil = () => {
  ...
  const asignarCordi = (animId) => {
    api
      .put(`/animadores/${animId}/roles`)
      .then((res) => {
        toast.success("Animador asignado como cordi");
        fetchPerfil(animId);
      })
      .catch((err) => {
        console.error("Error asignando cordi", err);
        const server = err?.response?.data;
        toast.error(
          server?.message || server || err.message || "Error al asignar cordi",
        );
      });
  };

  const quitarCordi = (animId) => {
    api
      .delete(`/animadores/${animId}/roles`)
      .then((res) => {
        toast.success("Animador removido de cordi");
        fetchPerfil(animId);
      })
      .catch((err) => {
        console.error("Error quitando cordi", err);
        const server = err?.response?.data;
        toast.error(
          server?.message || server || err.message || "Error al quitar cordi",
        );
      });
  };
        ...
        <div className="perfil-grid">
          <aside className="perfil-left">
            ...
            {/* <div className="badges">
              {perfil.division && (
                <span className="badge">Division {perfil.division}</span>
              )}
            ...
            <div className="actions">
              <button
                className="btn btn-asign"
                onClick={asignarCordi.bind(null, perfil._id || perfil.id)}
              >
                Asignar como cordi
              </button>
              {perfil.roles && perfil.roles.includes("cordi") && (
                <button
                  className="btn btn-remove"
                  onClick={quitarCordi.bind(null, perfil._id || perfil.id)}
                >
                  Quitar cordi
                </button>
              )}
